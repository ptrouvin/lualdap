...
** @param #5 Integer port number to use, default to LDAP_PORT(=389)
...
static int lualdap_open_simple (lua_State *L) {
	...
	lua_Integer lport = lua_tointegerx(L, 5);
	int port = LDAP_PORT;
	if( lport>=INT_MIN && lport<=INT_MAX && lport )
		port=lport;
